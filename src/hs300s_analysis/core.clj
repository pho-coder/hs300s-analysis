    ...
    (if hs300-download-finish?
      (do (let [summary-path (str path "/../summary/" dt)]
            ...
            (let [summary (analysis/summary-one-day path dt)]
              (utils/write-csv (:summary (:all summary))
                               (str summary-path "/all"))
              (log/info "write all csv")
              (utils/write-csv (:top-trans-amount (:all summary))
                               (str summary-path "/all.top-trans-amount"))
              (log/info "write all.top-trans-amount csv")
              (utils/write-csv (:summary (:morning summary))
                               (str summary-path "/morning"))
              (log/info "write morning csv")
              (utils/write-csv (:summary (:afternoon summary))
                               (str summary-path "/afternoon"))
              (log/info "write afternoon csv")
              (utils/write-csv (:summary (:volume-400 summary))
                               (str summary-path "/volume-400"))
              (log/info "write volume-400 csv")
              (utils/write-csv (:summary (:amount-10 summary))
